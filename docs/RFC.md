...
**Code identifiers renamed to English (2026-08-02), separate from the UI-string pass above.** Adding the `id` locale surfaced pre-existing Indonesian *code* identifiers — PHP variables and array keys, not just UI copy — mixed into otherwise-English service code: `DashboardService::summary()`'s return array (`stock_awal_qty`/`biaya_sdm`/`saldo_kas`/`gerai_aktif`/etc.) and local variables, `InventoryReportService::stockSummary()`'s return array (`qty_awal`/`value_akhir_cost`/etc.) and locals, and `Branch`'s `jenis` column. All renamed to English (`stock_opening_qty`, `staff_cost`, `cash_balance`, `active_branches`, `qty_opening`, `value_closing_cost`, `Branch.type`, …) — see git history for the full old→new key mapping if tracing an older commit's field names. `erp-dashboard.blade.php`'s `$t()`/`$s[]` accessors, both `dashboard.php` lang files' `tiles` keys (values untouched — the `id` file's Indonesian *translations* like `'Total Gerai'` stay exactly as-is, only the array *key* `total_gerai` became `total_branches`), and `Branch.jenis` (migration, model, factory, `BranchResource`, `SbBogorPdfSeeder`) all moved together. Business/domain terms that are the established bilingual vocabulary — *Gerai*, *Setoran Kas*, *Stock Opname*, category/description strings in `SbBogorPdfSeeder` that reproduce the source PDF's real Indonesian data — are content, not code identifiers, and were deliberately left alone; see the i18n section above and note 13 below for that distinction.
