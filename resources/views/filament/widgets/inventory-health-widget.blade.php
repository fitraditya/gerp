<div class="filament-widgets">
    <div style="display:flex;gap:1rem;align-items:center;">
        <div style="padding:1rem;border-radius:8px;background:#fff;border:1px solid #eee;flex:1;">
            <h3 style="margin:0;font-size:0.9rem;color:#333;">{{ __('widgets.inventory_health.total_products') }}</h3>
            <div style="font-size:1.6rem;font-weight:600;">{{ number_format($totalProducts) }}</div>
        </div>
        <div style="padding:1rem;border-radius:8px;background:#fff;border:1px solid #eee;flex:1;">
            <h3 style="margin:0;font-size:0.9rem;color:#333;">{{ __('widgets.inventory_health.low_stock', ['threshold' => $lowStockThreshold]) }}</h3>
            <div style="font-size:1.6rem;font-weight:600;color:{{ $lowStock ? '#d97706' : '#059669' }};">{{ number_format($lowStock) }}</div>
        </div>
        <div style="padding:1rem;border-radius:8px;background:#fff;border:1px solid #eee;flex:1;">
            <h3 style="margin:0;font-size:0.9rem;color:#333;">{{ __('widgets.inventory_health.negative_stock') }}</h3>
